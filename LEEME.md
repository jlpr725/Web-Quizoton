...
| `404.html` | Página de error para direcciones que no existen (Vercel la usa sola). Fondo oscuro y una «pregunta sorpresa» que funciona solo con CSS, sin JavaScript. **Sin mascota**, con el logo único y el mismo aviso de derechos del pie. Todas sus rutas empiezan por `/`, porque se muestra en cualquier dirección (por ejemplo `/guia/lo-que-sea`). Lleva `noindex`. |
...
├── 404.html               Página de error (rutas absolutas, sin mascota)
├── .vercelignore · favicon.ico · robots.txt · sitemap.xml   (.vercelignore también excluye este LEEME.md)
...
| **Video de YouTube** | `js/config.js` → `videoYoutubeId` (hoy `JdXDAb4uH9k`): solo el ID del video, 11 caracteres (lo que va después de `youtu.be/` o de `v=`). El video se inserta al pulsar «Ver el video de presentación», no antes. Vacío = el recuadro del video no se muestra. Si cambias de video, cambia también el `href` del botón `data-video-boton` en `index.html`, que abre el video en YouTube si el JavaScript no carga. En YouTube el video debe ser público u oculto y permitir insertarlo. |
